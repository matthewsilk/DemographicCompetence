---
title: "SupMat_RIS_FDtransmission"
author: "Matthew Silk"
date: "21/01/2020"
  ...
  for(jj in 1:length(betas)){
    ...
    for(j in 1:length(phi)){
      ...
      for(i in 2:dim(x.R)[2]){
          ...
          x.R[2,i]<-Su*phi[j]*beta*(x.R[2,i-1]/x.R[3,i-1])*x.R[1,i-1]+
            Su*phi[j]*tmp.r*(beta*(x.R[2,i-1]/x.R[3,i-1]))*x.R[1,i-1]+
            Su*phi[j]*x.R[2,i-1]+
            Su*phi[j]*tmp.r*psi*(beta*(x.R[2,i-1]/x.R[3,i-1]))*x.R[2,i-1]
          x.R[3,i]<-sum(x.R[1:2,i])
          
          #dd survival model
          x.S[1,i]<-tmp.su*(1-beta*(x.S[2,i-1]/x.S[3,i-1]))*x.S[1,i-1]+
            tmp.su*R*(1-beta*(x.S[2,i-1]/x.S[3,i-1]))*x.S[1,i-1]+
            tmp.su*R*psi*(1-beta*(x.S[2,i-1]/x.S[3,i-1]))*x.S[2,i-1]
          x.S[2,i]<-tmp.su*phi[j]*beta*(x.S[2,i-1]/x.S[3,i-1])*x.S[1,i-1]+
            tmp.su*phi[j]*R*(beta*(x.S[2,i-1]/x.S[3,i-1]))*x.S[1,i-1]+
            tmp.su*phi[j]*x.S[2,i-1]+
            tmp.su*phi[j]*R*psi*(beta*(x.S[2,i-1]/x.S[3,i-1]))*x.S[2,i-1]
...
#3. Pathogen prevalence for survival-based regulation
#4. Pathogen prevalence for reproduction-based regulation
saveRDS(fRESULTS.N.S, paste0("RISfNS_",Su,".rds"))
saveRDS(fRESULTS.N.R, paste0("RISfNR_",Su,".rds"))
saveRDS(fRESULTS.PR.S, paste0("RISfPRS_",Su,".rds"))
saveRDS(fRESULTS.PR.R, paste0("RISfPRR_",Su,".rds"))
